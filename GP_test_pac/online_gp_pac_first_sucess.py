...
print("Start O-SGPR, time-series data")
...
# osgpr_model.eval()
# with torch.no_grad():
#     mean, var = osgpr_model.predict_dis(init_x)
# P_dis_multi = MVN(deepcopy(mean), deepcopy(torch.diag(var)))
...
for t, (x, y) in enumerate(zip(X, Y)):
    ...
    for _ in range(2):
        ...
        # Q_mean, Q_var = osgpr_model.predict_dis(x)
        # Q_dis = Normal(Q_mean, torch.sqrt(Q_var))
        #
        # epsilon_square = 0.01
        # loss_exp_item = torch.div(torch.pow(torch.squeeze(y) - Q_dis.mean, 2),
...
print("End O-SGPR, time-series data")
